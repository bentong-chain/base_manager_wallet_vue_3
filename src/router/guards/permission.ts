 ...
 */
/** 登录后要跳转的目标路径，避免 redirect 指向 /login 造成循环 */
function getLoginRedirectTarget(to: {
  path: string;
  fullPath: string;
...
export function setupPermissionGuard() {
  ...
  router.beforeEach(async (to, _from) => {
    ...
    try {
      ...
      if (!isLoggedIn) {
        ...
        NProgress.done();
        const target = getLoginRedirectTarget(to);
  ...
  router.afterEach((to) => {
    ...
    // 记录最近访问：有 path 即尝试记录，title 优先 meta.title，否则用 name 或路径最后一段
    if (to.path) {
      const title =
