...
/**
 * 管理员角色列表（GET /api/v1/admin/role/list）
 * 仅传有值参数且使用明确基本类型，避免 reactive/undefined 导致后端参数校验或签名失败
 */
export function getAdminRoleList(params: AdminRoleQueryParams) {
...
/**
 * 角色详情（GET /api/v1/admin/role/{id}）
 */
export function getAdminRoleDetail(id: number | string) {
 ...
 */
export function createAdminRole(data: AdminRoleSaveRequest) {
 ...
 */
export function updateAdminRole(id: number | string, data: AdminRoleSaveRequest) {
  return request({
    url: `${ADMIN_ROLE_BASE_URL}/${id}`,
    method: "put",
    data,
 ...
 */
export function getAdminRolePermissionIds(id: number | string) {
  return request<any, number[]>({
...
export function assignAdminRolePermissions(
  id: number | string,
  data: AdminRoleAssignPermissionsRequest
) {
  return request({
